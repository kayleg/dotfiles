...
local function run(tools, action)
  ...
  table.sort(op_seq, function(a, b)
    local a_start, a_end = a:range_for_set_lines()
    local b_start, b_end = b:range_for_set_lines()
    return a_start < b_start or (a_start == b_start and a_end < b_end)
  end)
  ...
  pcall(init_buffer_diff, buffer, original_lines)
  for i = #op_seq, 1, -1 do
